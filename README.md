...
A clean, highly customizable, and ATS-friendly LaTeX Curriculum Vitae (CV) template with support for a profile picture and semantic data commands.

## How to Create Your Own CV

To build your own CV, you will primarily be editing the `cv.tex` file. 

1. Copy or download `cv.tex`, and `cv.sty`.
2. Open `cv.tex` in your preferred LaTeX editor.
3. Update the `\cvHeader`, and subsequent sections with your personal information, work experience, education, etc.
4. Compile the document to generate your `cv.pdf`.

Below are three popular ways to edit and compile your CV.
...
4. Click the **Upload** icon (top left of the file tree) and upload the following files from your computer:
   - `cv.tex`
   - `cv.sty`
5. Change the main document setting so Overleaf knows which file to compile. Click the **Menu** button (top left), look for the **Main document** dropdown, and select `cv.tex`. *(Alternatively, you can just rename `cv.tex` to `main.tex`)*.
...
5. Once the container is built and running, open `cv.tex` and compile using the integrated LaTeX tools provided by the container.
...
4. Open the folder containing your CV files in VS Code.
5. Open `cv.tex`.
